...
class datawrapp(object):
    def __init__(self,name,ra,rae,dec,dece,pflux,pfluxe,iflux,ifluxe):
	...
	if name == "tgss":
        ...
        self.iflux = iflux
        self.ifluxe = ifluxe
        self.keys = ['radec','ra','rae','dec','dece','pflux','pfluxe']
        if iflux:
...
def ReadData(database):
    ...
    if database == "tgss":
        ...
        np.array(data.e_Spk/1000),np.array(data.Sint/1000),\
    ...
    if database == "vssr":
        ...
        np.array(data.flux_74_mhz_error),[],[])
    return dataset
